...
def main():
    ...
    5層のSAMACTが手書き数字を学習し、推論するサンプルシナリオ
    ...
    normData = data / 16.0 # 0-16のデータなので、16で割ると0-1に正規化できる。
    ...
    hiddenLayer1 = SAMLayer(nN2, LayerProperty(a=3, p=0.75), Step(), Step(),
                            weightDist=RandomProperty('kaiming', 'normal'))

    # ハイパーパラメータa=3, p=0.75を指定。
    # 重みの逆伝搬時の活性化関数にStep関数を指定。
    # 教師信号生成時の活性化関数に恒等関数を指定。(特許取得)
    hiddenLayer2 = SAMLayer(nN3, LayerProperty(a=3, p=0.75), Step(), Linear(),
                            weightDist=RandomProperty('kaiming', 'normal'))
    hiddenLayer3 = SAMLayer(nN4, LayerProperty(a=3, p=0.75), Step(), Linear(),
                            weightDist=RandomProperty('kaiming', 'normal'))
    outputLayer = SAMLayer(nN5, LayerProperty(a=3, p=0.75), Step(), Linear(),
                           weightDist=RandomProperty('kaiming', 'normal'))
    ...
    ## 2-4. ニューラルネットワークの生成
    # 上記layerを渡してモデルを構築
    model = Sequential(inputLayer, decoder, [hiddenLayer1, hiddenLayer2, hiddenLayer3, outputLayer])
    ...
    # region 3. 学習
    ## 3-1. 学習プロパティの設定
    # 学習率eta, iotaの初期値を5, 2に設定。1エポックごとに減衰する。
    ...
    ## 3-3. 学習結果の保存
    # パラメータの学習結果を保存
    dumpName = f"digits_{nN1}_{nN2}_{nN3}_{nN4}_{nN5}_ntd{len(trainData)}.h5"
    model.Save(dumpName)
